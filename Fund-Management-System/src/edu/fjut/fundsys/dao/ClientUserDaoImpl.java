...
public class ClientUserDaoImpl extends HibernateDaoSupport implements
	...
	private DetachedCriteria getCriteriaByHelper(CLientUserQueryhelper helper) {
		DetachedCriteria criteria = DetachedCriteria
				.forClass(ClientUserTrans.class);
		...
		if (StringUtils.isNotEmpty(helper.getIsClientUser())) {
			criteria.createCriteria("clientUser").add(
					Restrictions.eq("ClientId", helper.getIsClientUser()));
		}
	...
	public List<ClientUserTrans> totalClientUserTrans(
		...
		try {
			...
			clientUserTransList = criteria.setFirstResult(5 * (beginIdx - 1))
	...
	public void addClientPicture(ClientPicture clientPicture) {
		this.getHibernateTemplate().save(clientPicture);

	}

	@Override
	public ClientPicture getClientPicture(String clientId) {
		return (ClientPicture) this.getSession()
				.createQuery("from ClientPicture where clientId=?")
				.setParameter(0, clientId).uniqueResult();
	}

	@Override
	public void updateClientPicture(ClientPicture clientPicture) {
		this.getHibernateTemplate().update(clientPicture);
