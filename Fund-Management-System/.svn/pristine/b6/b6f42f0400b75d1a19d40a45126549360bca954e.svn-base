...
/**
 * 基金交易Action 命名空间： /fundtrans
 * 
 * @author Guo 基金申购操作 基金赎回赎回 模拟天数变化，计算昨日收益，客户总盈利 获得近7日某基金利率变化
 */
public class FundTransInfoAction extends BaseAction {
	...
	/**
	 * 该方法模拟日期过了一天 计算持仓表昨日收益，持仓总价，以及每个客户总盈利 计算每个产品昨日收益,以及该用户拥有的每个基金产品的持仓总价
	 * 
	 */
	...
	public String loadRateRecord() {
		// fundId
		data = new HashMap<String, Object>();
		rateRecords = fundTransInfoService
				.rateRecords(Integer.parseInt(fundId));
	...
	public String loadClientHold() {
		// clientNo
		data = new HashMap<String, Object>();
		fundHolds = fundTransInfoService.loadFundHoldById(clientId);
	...
	/**
	 * 根据持仓ID获取详细信息： 持仓总价值，昨日收益，基金ID, 基金名称，基金类型，年转化率，成立日期
	 */
	...
	public String allFundRecord() {
		/*
		 * 所有的基金交易信息,组合查询 transId transType clientId如果是后台的话,可以回传空值 fundId pageNo
		 * clientUser.ClientId
		 */
		data = new HashMap<String, Object>();
		try {
			int page = Integer.parseInt(pageNo);
			System.out.println("6666:"+lastId);
			helper = new FundTransQueryHelper(transId, transType,clientUser.getClientId(),
					fundId);
